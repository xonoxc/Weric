...
export const RecommendationService = Context.GenericTag<RecommendationService>(
...
export const RecommendationServiceLive = Layer.effect(
  RecommendationService,
  Effect.gen(function* () {
    ...
    const generateFeed = (
      ...
      return Effect.gen(function* () {
        const { data: stories, total } = yield* storyRepo
          ...
          .pipe(
            Effect.mapError(toScoringError("Failed to fetch published stories"))
          )
        ...
        const interactions = yield* interactionRepo
          .findByUser(userId)
          .pipe(
            Effect.mapError(toScoringError("Failed to fetch user interactions"))
          )
        ...
        const scored = scorer.scoreMany(stories, interests, interactedStoryIds)
